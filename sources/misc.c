...
/*   Updated: 2016/05/10 16:15:33 by bsouchet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int     check_point_edge(t_var *v)
{
	int i;
	int j;
	int c;
	...
	c = 0;
	for (i = 0, j = ((v->num * 2) - 1); i < (v->num * 2); j = i++)
	{
		if (((v->pts[i][1] >= v->y) != (v->pts[j][1] >= v->y)) &
		(v->x <= (v->pts[j][0] - v->pts[i][0]) * (double)(v->y - v->pts[i][1]) /
		(double)(v->pts[j][1] - v->pts[i][1]) + v->pts[i][0]))
...
static void     get_vertex_full(t_var *v, int i, double x, double y)
{
	v->a = 0.0;
	v->add = (2 * PI) / v->num;
	while (i < v->num)
...
static void     get_vertex_edge(t_var *v, int i, int rad)
{
	int max = i;
	double x = 0.0;
	double y = 0.0;
	...
	while (i < (max + v->num))
	{
		...
		if (v->num % 2)
		{
			if (odd_polygon_check(v, i + 1) == 1)
			{
				v->pts[i][0] = (round((rad + (v->odd * rad / 100)) * x) + v->cx);
				v->pts[i][1] = (round((rad + (v->odd * rad / 100)) * y) + v->cy);
			}
			else
			{
				v->pts[i][0] = (round((rad + (v->even * v->rad / 100)) * x) + v->cx);
				v->pts[i][1] = (round((rad + (v->even * v->rad / 100)) * y) + v->cy);
			}
		}
		else
		{
			v->pts[i][0] = ((i + 1) % 2) ? (round((rad + (v->odd * rad / 100)) * x) + v->cx) :
			(round((rad + (v->even * rad / 100)) * x) + v->cx);
			v->pts[i][1] = ((i + 1) % 2) ? (round((rad + (v->odd * rad / 100)) * y) + v->cy) :
			(round((rad + (v->even * rad / 100)) * y) + v->cy);
		}
		v->a += v->add;
		apply_transforms(v, i);
		printf("i : %i --- x : %i --- y : %i\n", i, v->pts[i][0], v->pts[i][1]);
		i++;
	}
}
...
void            draw_edge_polygon(t_var *v)
{
	...
	while (i < (v->num * 2))
		v->pts[i++] = (int *)malloc(sizeof(int) * 2);
	get_vertex_edge(v, 0, v->rad);
	get_vertex_edge(v, v->num, v->rad - 10);
	while (++v->y <= WIN_H - 1)
	{
		v->x = 213;
		while (++v->x <= WIN_W - 1)
			if (check_point_edge(v) == 1)
				put_pixel(v, OBJ_COLOR, 0);
	}
	i = 0;
	while (i < (v->num * 2))
